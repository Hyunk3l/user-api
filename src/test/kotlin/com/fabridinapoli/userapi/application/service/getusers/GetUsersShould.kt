...
class GetUsersShould {
    ...
    fun `return a list of valid users`() {
        ...
        val expectedUsers = GetUsersResponse(VALID_USER_ID, VALID_NAME, VALID_SURNAME)
        val getUsers = GetUsers(inMemoryUserRepository)

        val users = getUsers.execute()

        users.blockFirst() shouldBeEqualTo expectedUsers
    }
    ...
    companion object {
        ...
        private const val VALID_EMAIL = "anemail@test.com"
        private val VALID_USER_ID = UUID.randomUUID().toString()
    }
}